...
export const AnnouncementBanner: React.FC<AnnouncementBannerProps> = ({
  ...
  return (
    <div className="relative z-30 pt-24 sm:pt-28 pb-3 px-4 sm:px-6 lg:px-8">
      <div className="max-w-5xl mx-auto">
        <div 
          onClick={() => onSelectAnnouncement ? onSelectAnnouncement(announcement) : onViewAll?.()}
          className="group relative flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 p-3 sm:px-5 sm:py-2.5 rounded-2xl glass-3d-base hover:glass-3d-elevated cursor-pointer transition-all duration-200 overflow-hidden shadow-md shadow-indigo-500/5 hover:shadow-lg hover:shadow-indigo-500/10 border border-white/90"
        >
          ...
          <div className="flex items-center gap-3 pl-3 sm:pl-2">
            <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-xl bg-indigo-50/90 text-indigo-700 border border-indigo-200/90 shadow-xs group-hover:scale-105 transition-transform">
